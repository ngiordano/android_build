...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/pac/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the PacMan model name
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.pac.device=$PAC_BUILD");
    then
